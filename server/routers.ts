...
const ostrcRouter = router({
  ...
  importOstrcData: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const playerByName = new Map<string, typeof playersList[0]>();
      const playerByNumber = new Map<number, typeof playersList[0]>();
      playersList.forEach(p => {
        playerByName.set(p.name.trim(), p);
        playerByNumber.set(p.number, p);
      });
